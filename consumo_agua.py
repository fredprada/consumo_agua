...
# Ranking geral (Agora abaixo dos indicadores)
if usuario_id:
    ...
    if response.status_code == 200:
        ...
        if not df_ranking.empty:
            ...
            media_usuario = ranking[ranking["usuario_id"] == usuario_id]["media_diaria"].values[0]

            st.metric(label="Sua Posição no Ranking", value=posicao_usuario)
            st.metric(label="Seu Consumo Total", value=f"{total_usuario}ml")
            st.metric(label="Média Diária", value=f"{media_usuario:.0f}ml")
