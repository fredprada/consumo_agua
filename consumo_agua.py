...
def obter_historico():
    ...
    if response.status_code == 200:
        ...
        if not df.empty:
            ...
            df["data"] = df["data_hora"].dt.date  # Criando a coluna data logo aqui
        return df
    return pd.DataFrame()
...
if not historico.empty and "data" in historico.columns:
...
# 🔹 7️⃣ Histórico de Consumo (FILTRADO POR USUÁRIO)
if usuario_id and filtrar:
