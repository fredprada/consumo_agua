...
# 🔹 1️⃣ Seleção de usuário existente
usuarios = obter_usuarios()
usuario_selecionado = st.selectbox("Selecione um usuário:", usuarios, index=0 if usuarios else None)

filtrar = st.button("🔍 Filtrar")

# 🔹 2️⃣ Input de consumo (para usuário existente ou novo)
st.subheader("➕ Registrar Consumo")

novo_usuario = st.text_input("Novo usuário (opcional):")
usuario_id = novo_usuario.strip() if novo_usuario.strip() else usuario_selecionado
...
if st.button("Registrar"):
    if usuario_id:
        ...
        if response.status_code == 201:
            st.success(f"Registrado: {quantidade_ml}ml para {usuario_id}!")
        else:
            st.error("Erro ao registrar consumo.")
    else:
        st.error("Selecione ou digite um nome para o usuário.")
...
# 🔹 4️⃣ Ranking de Consumo Médio Diário
st.subheader("🏆 Ranking de Consumo Médio Diário")

historico = obter_historico()
if not historico.empty:
    # Garantir que a coluna 'data_hora' esteja no formato datetime antes de criar 'data'
    historico["data_hora"] = pd.to_datetime(historico["data_hora"], errors='coerce')

    if historico["data_hora"].isna().any():
        st.error("Erro ao processar datas. Verifique os registros.")
    else:
        historico["data_hora"] = historico["data_hora"].dt.tz_convert(UTC_MINUS_3)
        historico["data"] = historico["data_hora"].dt.date

        # Agrupar consumo diário por usuário
...
if usuario_id and filtrar:
    ...
    if not historico_usuario.empty:
        ...
        consumo_hoje = historico_usuario[historico_usuario["data"] == hoje]["quantidade_ml"].sum()
        consumo_diario = historico_usuario.groupby("data")["quantidade_ml"].sum()
