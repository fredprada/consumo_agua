...
if usuario_id and filtrar:
    ...
    if not historico.empty:
        ...
        # Cálculo do consumo necessário por hora restante
        horas_restantes = max(1, 24 - agora.hour)  # Evita divisão por zero
        ...
        col3.metric(label="Necessário por Hora", value=f"{consumo_por_hora:.0f}ml/h | Faltam {restante_meta}ml")

    else:
        st.write("Nenhum registro encontrado.")

# 🔹 5️⃣ Ranking de Consumo Médio Diário (agora sem filtro)
...
if usuario_id and filtrar:
    ...
    if not historico_usuario.empty:
        ...
        fig_media_hora = px.bar(
            media_hora, x="hora", y="quantidade_ml", title="Média de Consumo por Hora",
            labels={"hora": "Hora do Dia", "quantidade_ml": "Média de Consumo (ml)"},
