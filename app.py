...
if usuario_id and filtrar:
    ...
    if not historico.empty:
        ...
        # 📊 Gráfico: Consumo por Dia (barra do dia atual em cinza)
        consumo_por_dia = historico_usuario.copy()
        consumo_por_dia["cor"] = consumo_por_dia["data"].apply(lambda x: "gray" if x == hoje else "#636EFA")

        fig_dia = px.bar(consumo_por_dia, x="data", y="quantidade_ml", title="Consumo Total por Dia", text_auto=True)
        fig_dia.update_traces(marker=dict(color=consumo_por_dia["cor"]))

        st.plotly_chart(fig_dia)

        # 📊 Gráfico: Evolução do Consumo Diário (%) da Meta (com linha vertical no dia atual)
