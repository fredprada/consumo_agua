...
# 📈 Evolução do % da Meta Diária
st.subheader("📊 Evolução do % da Meta Diária")

if not historico.empty:
    consumo_diario_total = historico.groupby("data")["quantidade_ml"].sum().reset_index()
    consumo_diario_total["% da Meta"] = (consumo_diario_total["quantidade_ml"] / META_DIARIA) * 100

    fig_meta = px.line(
        consumo_diario_total,
        x="data",
        y="% da Meta",
        title="Evolução do % da Meta Diária",
        markers=True,
        line_shape="spline"
    )
